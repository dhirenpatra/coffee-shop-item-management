...
public class ProductHandler {
    ...
    public Mono<ServerResponse> updateProduct(ServerRequest serverRequest) {
        ...
        return productMonoToBeUpdated.zipWith(existingProductById,
                (productMono, existingProduct) -> new Product(existingProduct.getProductId(),
                        productMono.getProductName(), productMono.getPrice()))
    ...
    public Mono<ServerResponse> productEvents(ServerRequest serverRequest) {
        ...
        Flux<ProductEvent> productEventFlux = Flux.interval(Duration.ofSeconds(1)).map(item -> {
            return new ProductEvent(UUID.randomUUID().toString(), Instant.now(), item);
        });
